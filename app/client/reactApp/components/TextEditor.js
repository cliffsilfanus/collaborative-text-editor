...
export default class TextEditor extends Component {
  constructor(props) {
    ...
    this.onChange = editorState => {
      props.socket.emit("changeDoc", {
        ...
        selectData: editorState.getSelection()
      });
    ...
    this.changeDoc = ({ data, selectData }) => {
      const currState = EditorState.createWithContent(convertFromRaw(data));
      let currSelect = SelectionState.createEmpty();
      currSelect = currSelect.merge(selectData);
      this.setState({
  ...
  _toggleList(type) {
    this.onChange(
      RichUtils.toggleBlockType(this.state.editorState, `${type}-list-item`)
    );
  }
